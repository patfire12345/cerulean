export const dataVN = {
  nav: {
    titles: ["TRANG TRỦ", "GIỚI THIỆU", "DỰ ÁN", "TUYỂN DỤNG", "LIÊN HỆ"],
    links: ["", "trangtru", "duan", "tuyendung", "lienhe"],
  },
  home: {
    text: "Tổ chức Cerulean hướng tới xây dựng một tương lai bền vững cho tất cả trẻ em và thanh thiếu niên. Tổ chức hoạt động vì một thế giới nơi mỗi đứa trẻ được lắng nghe, được đối xử với sự tôn trọng và được yêu thương.",
    imagetext: "CARE. \nRESPECT. \nEMPOWERMENT.",
    button: "NHẬN BẢN TIN NGAY!",
    image: "/trangtru_img.png",
    alt: "trangtru",
  },
  about: {
    title: "GIỚI THIỆU",
    aboutUs:
      "Tổ chức phi lợi nhuận Cerulean được thành lập vào năm 2020 với mục tiêu quảng bá các nền văn hóa Pháp, Anh và Việt Nam. Đến nay, Cerulean đã thành công kết nối hơn 300 học viên với các giảng viên dạy tiếng Anh và tiếng Pháp đến từ nhiều nước khác nhau trên thế giới. \n \n Nhận thấy tầm quan trọng của giáo dục đối với thế hệ tương lai, Cerulean tin rằng đem lại một nền giáo dục chất lượng cho tất cả là cơ sở để tạo ra một thế giới hòa bình và thịnh vượng. Sứ mệnh mới của tổ chức là hỗ trợ tất cả trẻ em trên khắp Việt Nam tiếp cận một nền giáo dục chất lượng. \n \n Tổ chức Cerulean hướng tới mang lại nhiều cơ hội học tập chất lượng, nhằm giúp trẻ em Việt Nam có kiến thức và kỹ năng cần thiết cho một cuộc sống lành mạnh và tốt đẹp hơn.",
    subtitles: {
      core: "CHÚNG TÔI TIN VÀO",
      story: "KHỞI ĐẦU",
      testimonials: "NHẬN XÉT",
    },
    ourCoreValues: [
      [
        "CARE",
        "Chúng tôi hỗ trợ lẫn nhau phát triển trong môi trường bao trùm, nơi mà ai cũng được lắng nghe, được thấu hiểu và được đánh giá tích cực.",
        "QUAN TÂM",
      ],
      [
        "RESPECT",
        "Chúng tôi làm việc với nhau với thái độ tôn trọng vì chúng tôi khuyến khích sự bao trùm và sự đa dạng, cũng như lan tỏa tình thương và tạo cảm giác được đón nhận cho mọi người.",
        "TÔN TRỌNG",
      ],
      [
        "EMPOWERMENT",
        "Chúng tôi trân trọng khả năng làm việc, khát vọng và sự ham muốn học hỏi của nhau. Hơn thế nữa, chúng tôi khuyến khích tính tự lập.",
        "TỰ CHỦ",
      ],
    ],
    testimonialsText: [
      [
        "Mình đến vì Project 1,\nMình ở lại vì Cerulean.",
        "Vy Vy - Ban Content",
      ],
      [
        "Mình đã may mắn được gặp các người bạn trên khắp đất nước có chung niềm đam mê với văn hoá, giáo dục và ngôn ngữ.",
        "Hồng Hạnh - Ban Content",
      ],
      [
        "Cerulean cứ như một gia đình nơi mọi người vừa chơi vừa học í.",
        "Nhi - Ban Đối Ngoại ER",
      ],
      [
        "Lúc mình vào Cerulean thì đúng như một tờ giấy trắng luôn [...]. Thế nhưng mọi người vô cùng nhiệt tình giúp đỡ mình.",
        "Hương Giang - Ban PR",
      ],
      [
        "Đúng là 'đường dài mới biết ngựa hay'. Mình đã học hỏi được nhiều điều từ mọi người để phát triển bản thân từng ngày.",
        "Nguyên Hà - Ban Nhân sự HR",
      ],
      [
        "Môi trường Cerulean siêu siêu tuyệt vời luôn!",
        "Ngọc Kiều - Ban Đối Ngoại ER",
      ],
      [
        "Các thành viên luôn vì nhau mà cố gắng, san sẻ kinh nghiệm giúp cho dự án ngày càng thành công.",
        "Linh - Ban Nhân sự HR",
      ],
      [
        "Cerulean không chỉ mang tới những lợi ích tốt đẹp cho trẻ em mà còn tạo ra [...] nhiều cơ hội cho các bạn trẻ.",
        "Phong Lưu - Design",
      ],
    ],
  },
  projects: {
    title: "Projects",
    tutor: "TUTORING PROJECTS",
    reverie: {
      title: "REVERIE 2022",
      text: "Inspired by Western classical art, Cerulean decided to bring the REVERIE project to life on August 10, 2022, at Thanh Tam Shelter with a desire to spread and share cultural diversity with the community while offering children an opportunity to discover their identities, pursue their passions, and simply find joy in each and every brushstroke. \n \n REVERIE is a Fauvism painting session and exhibition with all of its paintings beautifully crafted by talented young artists from the Thanh Tam Shelter who got to freely immerse themselves in marvelous French fairy tales while creating such masterpieces. Through the event, Cerulean was able to deliver meaningful values and bring volunteer work closer to the community as well as the children, thus connecting souls that share the same idea, creating a memorable day. \n \n Moreover, Cerulean and REVERIE have succeeded in providing the Shelter’s young artists a chance to learn more about themselves in which, eventually, we could all see and admire the artistic beauty of France through their little innocent and purely adorable strokes. As a result, we shall be able to take a break from the amount of stress and pressure that life has to offer while also being able to better listen and understand our inner selves more deeply. ",
      image: "/reverie_img.png",
    },
    bonfire: {
      title: "BONFIRE 2020",
      image: "/bonfire_img.png",
    },
  },
  join: {
    title: "Join Us",
    image: "/join_img.jpg",
    alt: "joinus",
    description: {
      start:
        "If you are looking to make a difference and wish to work in a dynamic, youthful and professional environment, join our cause! You’ll get to: ",
      middle: [
        "Participate in a dynamic, multicultural, professional and fun environment",
        "Enhance your knowledge and improve your social skills",
        "Expand your network, connecting with fellows and professional seniors from all over the world",
        "Have the opportunity to learn new languages for free at work",
        "Receive a certificate once having completed 80% of work",
        "Receive a recommendation letter written by Core Team",
      ],
      end: "The Cerulean family always welcomes you!",
    },
    button: "APPLY NOW",
  },
  contact: {
    infotext: "Để biết thêm thông tin, ",
    socialtext:
      "Vui lòng truy cập các trang mạng xã hội của Tổ chức Cerulean\nhoặc trao đổi trực tiếp qua email với chúng tôi.",
    links: [
      "https://www.linkedin.com/company/cerulean-organization/",
      "mailto:cerulean.org@gmail.com",
      "https://www.facebook.com/c3rulean.org?mibextid=ZbWKwL",
      "https://instagram.com/cerulean_org?igshid=MzRlODBiNWFlZA==",
    ],
    icons: ["/linkedin.png", "/email.png", "/facebook.png", "/instagram.png"],
  },
};
